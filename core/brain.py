...
def init_lora_engine():
    ...
    try:
        ...
        if not os.path.exists(BASE_MODEL_PATH):
            print(f"[LoRA] Base model path {BASE_MODEL_PATH} not found. Running in Fallback-only mode.")
            lora_available = False
            return False
        ...
        try:
            ...
            print(f"\n[LoRA] Failed to load Academic Brain weights. Error: {e}")
            lora_available = False
            return False

        # 4. Attach LoRA Adapter
        if not os.path.exists(ADAPTER_PATH):
            print(f"[LoRA] Adapter path {ADAPTER_PATH} not found.")
            lora_available = False
            return False
...
def ask_lora_brain(user_input):
    ...
    try:
        ...
        # Stop behavior: Cut generation when a new question starts (contains "?")
        if "?" in response:
            response = response.split("?")[0].strip()

        # Add Stop Behavior for unrelated content / topic switching
...
def unified_brain_pipeline(command):
    """
    Main Brain Pipeline: LoRA Primary -> Gemini Fallback.
    Rule: Academic focus, no extra intent classification unless hardware.
    """
    global conversation_history
    raw_query = command.strip()
    query = normalize_query(raw_query)
    
    print(f"[Brain] Processing: {query}")

    try:
        # 1. SPECIALIZED HANDLERS (Math/KB)
        # Try math first for accuracy
        ...
        # 2. GENERATION LAYER (Primary: LoRA Academic Brain)
        if lora_available:
            print("SOURCE: LoRA (Primary)")
        ...
        # 3. FALLBACK LAYER (Gemini API)
        print("SOURCE: GEMINI (Fallback)")
        system_prompt, contextual_query = build_context(query)
        full_gemini_query = f"{system_prompt}\n\n{contextual_query}"
        
        try:
            from core.gemini_engine import ask_gemini
            response = ask_gemini(full_gemini_query)
            if response:
                update_history(query, response)
                return response
        except Exception as e:
            print(f"[Brain] Gemini Fallback Failed: {e}")

        return "I apologize, but my academic engine is currently under maintenance. Please try again in a moment."

    except Exception as e:
        import traceback
        print(f"[Brain] CRITICAL ERROR: {e}")
        traceback.print_exc()
        return "I encountered a cognitive error. Please try rephrasing your request."

# Initialize LoRA engine upon module load
init_lora_engine()
